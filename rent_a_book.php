...
<body>
    ...
    <div class="gridgallery">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['searchbutton'])) {
            ...
            //echo $searchinput;
            // Build a simple SQL query with LIKE and %
            $searchinput = strtolower($searchinput); // Ensure input is lowercase

            $searchquery = "SELECT * FROM `book_details_tab` WHERE 
            (LOWER(REPLACE(REGEXP_REPLACE(book_name, '[^a-zA-Z0-9]', ''), ' ', '')) LIKE '%$searchinput%' OR
            author_name LIKE '%$searchinput%' OR
            category LIKE '%$searchinput%' OR
            language LIKE '%$searchinput%')
            AND no_of_copies > 0
